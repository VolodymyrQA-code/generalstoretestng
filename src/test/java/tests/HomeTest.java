...
public class HomeTest extends BasePage {
    ...
    @Test @Order(1) @Description("Splash screen should be visible")
    ...
    @Test @Order(2) @Description("Перевіряємо, що заголовок тулбара відображається і має текст \"General Store\"")
    public void testToolbarTitle() {
        home.verifyToolbarTitle();
    }

    @Test @Order(3) @Description("Перевіряємо, що заголовок для вибору країни відображається і відповідає очікуваному тексту")
    public void testCountryTitle() {
        home.verifyCountryTitle();
    }
    @Test @Order(4) @Description("Перевіряємо, що заголовок поля для введення імені користувача відображається")
    public void testYourNameTitle() {
        home.verifyYourNameTitle();
    }

    @Test @Order(5) @Description("Перевіряємо, що заголовок для вибору статі відображається")
    public void testGenderTitle() {
        home.verifyGenderTitle();
    }

    @Test @Order(6) @Description("Перевіряємо, що підпис для чоловічого радіо-батону відображається")
    public void testMaleRadioTitle() {
        home.verifyMaleRadio();
    }

    @Test @Order(7) @Description("Перевіряємо, що підпис для жіночого радіо-батону відображається")
    public void testFemaleRadioTitle() {
        home.verifyFemaleRadio();
    }

    @Test @Order(8) @Description("Перевіряємо, що кнопка \"Let's Shop\" відображається і має правильний текст")
    public void testLetsShopBtnTitle() {
        home.verifyLetsShopBtnTitle();
    }

    @Test @Order(9) @Description("Перевіряємо, що країна зі списку обрана") 
    ...
    @Test @Order(11) @Description("Перевіряємо, що введене ім'я відображається у полі після введення")
    public void testVerifyEnteredName() {
        String userName = ConfigReader.getProperty("userName"); // читаємо з config.properties
        home.enterAndVerifyName(userName);
    }

    @Test @Order(12) @Description("Перевіряємо, що жіночий радіо-батон обраний, а чоловічий не обраний")
    public void testVerifyFealeRadioButtonSelected() {
        home.verifyFemaleRadioSelected();
        home.verifyMaleRadioNotSelected();
    }

    @Test @Order(13) @Description("Перевіряємо, що чоловічий радіо-батон обраний, а жіночий не обраний")
